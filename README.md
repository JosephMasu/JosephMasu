<h2 align="left">Hi 👋, I'm Masu Joseph</h1>

[![Joseph_Masu Linkedin](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/masu-joseph-503855216/)
[![Joseph_Masu Twitter](https://img.shields.io/badge/Twitter-1DA1F2?style=for-the-badge&logo=twitter&logoColor=white)](https://x.com/MasuJoe)
[![Joseph_Masu Medium](https://img.shields.io/badge/Medium-000000?style=for-the-badge&logo=medium&logoColor=white)](https://medium.com/@josephmuhindo089)

I’m a Software Engineer with 6+ years of experience building web and mobile applications across fintech, 
SaaS, and digital products. I work primarily with TypeScript, JavaScript, React, Next.js, React Native, 
Node.js, NestJS, and Spring Boot, and have experience with APIs, microservices, databases, cloud infrastructure, 
and CI/CD. I enjoy turning ideas and complex requirements into reliable, user-friendly products, leading frontend teams, 
solving technical problems, and continuously learning new technologies. Pragmatic, product-minded, and focused on building software that works.

**Domain areas:** , SaaS, APIs, Regulated industries (Fintech), Fintech, Payments systems, web3.

## CORE TECHNICAL SKILLS
**Languages:** JavaScript, TypeScript, Java, and SQL.

**Frontend:** React.js, Angularjs ,React Native, Next.js.

**Backend:**  Node.js, NestJS, Spring Boot, and GraphQL

**DevOps & Cloud:** AWS, Docker, Kubernetes, CI/CD,Kafka

**Databases:** PostgreSQL, MySQL, MongoDB, firebase


**Tools:** Git, GitHub, GitLab, JUnit, Postman, Redux

**Practices:** Agile, Microservices, Debugging

# Experience
## Software Engineer
**Shwary, Inc**
09/2025 - present | Remote

## Full Stack Developer
**VisionMate**  
05/2025 -  present | Remote

## Frontend Engineer Team lead 
**Necta**  
04/2025 -  01/2026 | Remote

## Frontend Engineer 
**QuariLabs**  
*06/2024 – 03/2025 | Remote*  

## Frontend Engineer Team lead 
**Velix Protocol**  
*08/2023 – 03/2025 | Remote*  

## Full Stack Developer  
**Fasto Services**  
*02/2020 – 04/2024 |  Kampala*


 **📧 How to reach me: <a href="galiencodes13@gmail.com">josephmuhindo089@gmail.com<a/>**
